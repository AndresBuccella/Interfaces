document.addEventListener("DOMContentLoaded", function () {
    ...
    form_regist.querySelector("#correo").addEventListener("input", function () { validate_correo("correo"); });
    form_regist.querySelector("#contraseña").addEventListener("input", function () { validate_password("contraseña") });
    ...
    const regLoader = document.querySelector('.btn-registrarse .loader');

    //Resultado cuando la animacion del loader termina
    regLoader.addEventListener('animationend', (event) => {
    ...
    function validate_correo(id) {
        let correo = document.querySelector(`#${id}`);
    ...
    function validate_password(id) {
        let contraceña = document.querySelector(`#${id}`);
    ...
    let btn_login = document.querySelector("#iniciar-sesion"); //Boton de registro
    btn_login.addEventListener("click", validar_login); //El boton chequea que se cumplan los campos

    const inicLoader = document.querySelector('.btn-iniciar-sesion .loader');


    let form_inic = document.querySelector("#formulario-iniciar-sesion");
    form_inic.querySelector("#email").addEventListener("input", function () { validate_correo("email"); });
    form_inic.querySelector("#init-contraceña").addEventListener("input", function () { validate_password("init-contraceña"); });

    function validar_login() {
        btn_login.classList.remove("cheking");
        btn_login.classList.remove("registro-ok");
        btn_login.classList.remove("registro-not-ok");
        inicLoader.style.display = 'block';
        btn_login.classList.add("cheking");

        status = true;
        if (!validate_correo("email")) status = false;
        if (!validate_password("init-contraceña")) status = false;
    }

    //Resultado cuando la animacion del loader termina
    inicLoader.addEventListener('animationend', (event) => {
        btn_login.classList.remove("cheking");
        if (status) {
            btn_login.classList.add("registro-ok");
            btn_login.addEventListener('animationend', (event) => {
                window.location.href = "html/home.html"; //Si el resultado de la validacion de ok te envia al index al terminar la animacion del boton registro
            });
        }
        else btn_login.classList.add("registro-not-ok");
    });
})